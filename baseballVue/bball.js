const baseballPlayers = [
  ...
  const playerNames = baseballPlayers.filter(names =>{
        return names.team
  })
  ...
  ///// Can't return the Indians
    const indianPlayers = baseballPlayers.map(item => {
       if(item.name.team === "Cleveland Indians"){
           return item
       }
   })
  ...
  //  examples -> [ { name: "Barry Bonds", team: "San Francisco Giants" }, { name: "Hank Aaron", team: "Atlanta Braves"}, etc..]
    ...
    const nameTeams = baseballPlayers.map(item => {
        return item.name + " who plays for: "+ item.team
    })

    //console.log(nameTeams)
  // 4) Create a function that returns the average of all players HR (for loop)
  
   const avgHr = baseballPlayers.map(item =>{
       let hr = 0
     for(let x=0; x < item.length; x++){
         hr += item[x].hr
     }
     let avg = hr/item[x].hr
   })
  ...
  const allStarIndians = baseballPlayers.filter(items =>{
           return items.hr > 575
  })

  console.log(allStarIndians)
  // 7) Use Filter & Map to return an array of just the Names of the Cleveland Indian Players with HR's greater than 575.
  
  const hrGreaterThan = baseballPlayers.filter(item =>{
      return item.hr > 575
  }).map(name =>{
      return {name: name}
  })

  console.log(hrGreaterThan)
